    ...
    <el-form label-position="right" :model="query" class="form p_4" label-width="120">
      ...
      <el-row class="rows row2">
         ...
         <el-col :xs="24" :sm="10" :lg="8" class="search_btn_1 search_btn_wrap_1 btns">
            ...
            <el-button v-if="$check_action('/complaint_records/table','del') || $check_action('/complaint_records/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
                        </el-col>
	    ...
	    <el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
	                <el-table-column fixed type="selection" tooltip-effect="dark" width="55">
                      ...
                      {{ get_user_tenant_user(scope.row['tenant_user']) }}
                                              ...
                                              <el-table-column prop="date_of_complaint" @sort-change="$sortChange" label="投诉日期"                                v-if="$check_field('get','date_of_complaint')" min-width="200">
                                      <template slot-scope="scope">
                      {{ $toTime(scope.row["date_of_complaint"],"yyyy-MM-dd") }}
                    </template>
                                </el-table-column>
      ...
      <el-table-column sortable prop="update_time" label="更新时间" min-width="200">
        <template slot-scope="scope">
          {{ $toTime(scope.row["update_time"],"yyyy-MM-dd hh:mm:ss") }}
                ...
                <el-table-column fixed="right" label="操作" min-width="120" v-if="$check_action('/complaint_records/table','set') || $check_action('/complaint_records/view','set') || $check_action('/complaint_records/view','get')
					 ">
        

      <template slot-scope="scope">
        <router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
                     v-if="$check_action('/complaint_records/table','set') || $check_action('/complaint_records/view','set') || $check_action('/complaint_records/view','get')"
                                 :to="'./view?' + field + '=' + scope.row[field]"
                                 size="small">
          <span>详情</span>
        </router-link>
                                        		  		  		  	<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="changeSigning(scope.row, scope.$index)" v-if="$check_option('/complaint_records/table', 'examine')">
		  		<span>审核</span>
		  	</el-button>
		        </template>
    </el-table-column>
  ...
  import mixin from "@/mixins/page.js";
    ...
    data() {
      return {
        ...
        showModal: false,
        // 获取数据地址
        url_get_list: "~/api/complaint_records/get_list?like=0",
        url_del: "~/api/complaint_records/del?",
					...
					if (valid) {
						...
						if (this.batchAllState) {
							for (var i = 0; i < this.batchAllList.length; i++) {
								let url = "~/api/complaint_records/set?complaint_records_id=" + this.batchAllList[i]["complaint_records_id"];
							...
							var url = "~/api/complaint_records/set?complaint_records_id=" + this.verifyItem['complaint_records_id']
							_this.$post(url, {
								'examine_state': _this.verifyItem.examine_state,
																'examine_reply': _this.verifyItem.examine_reply
															}, (json, status) => {
