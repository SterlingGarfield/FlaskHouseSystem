<template>
	<div class="diy_details_box diy_div_housing_information">
		<div class="warp diy_warp">
			<div class="container">
				...
				<div class="row row_det">
						<div v-if="this.$store.state.user.user_id" class="btns_add col-12 col-sm-6">
							<div v-if="$check_action('/housing_information/details','set')">
								<!-- 点赞按钮 -->
								<div
										v-if="state_praise"
			...
			<div class="details_btn_wrap">
																											<button class="btn btn-primary details_btn" v-if="$check_action('/appointment_to_see_the_room/edit','add') && !appointment_to_see_the_room_limit" @click="to_form('/appointment_to_see_the_room/edit')" ><span> 在线预约</span> </button>
																		<button class="btn btn-primary details_btn" v-if="$check_action('/financial_statements/edit','add') && !financial_statements_limit" @click="to_form('/financial_statements/edit')" ><span> 财务报表</span> </button>
																										</div>
						<!-- 富文本 -->
...
<script>
	...
	export default {
		...
		data() {
			return {
				...
				list_user_landlord_user: [],
									state_praise:false,
				state_collect:false,
							appointment_to_see_the_room_limit: false,
						financial_statements_limit: false,
						};
		},
		methods: {
																/**
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					this.state_collect = true;
								query.title = this.obj.house_title
																								query.img = this.obj.house_picture
      ...
      check_collect() {
        var user_id = this.$store.state.user.user_id;
        var housing_information_id = this.obj.housing_information_id;
        this.$get('~/api/collect/count?', {
          user_id,
          source_table: "housing_information",
          source_field: "housing_information_id",
          source_id: housing_information_id
        }, (res) => {
          this.state_collect = res.result
        });
      },
			  get_appointment_to_see_the_room_limit(){
	  	let param = {
	  		source_table: "housing_information",
	  		source_id: this.obj.housing_information_id,
	  		source_user_id: this.user.user_id
	  	};
	  	if(this.obj.appointment_to_see_the_room_limit_times > 0){
	  		this.$get("~/api/appointment_to_see_the_room/count?",param,(result)=>{
	  			if(result){
	  				if(result.result >= this.obj.appointment_to_see_the_room_limit_times){
	  					this.appointment_to_see_the_room_limit = true;
	  				}else{
	  					this.appointment_to_see_the_room_limit = false;
	  				}
	  			}
	  		})
	  	}else{
		...
		this.obj.source_user_id = param.source_user_id;
		  ...
		  get_financial_statements_limit(){
	  	let param = {
	  		source_table: "housing_information",
	  		source_id: this.obj.housing_information_id,
	  		source_user_id: this.user.user_id
	  	};
	  	if(this.obj.financial_statements_limit_times > 0){
	  		this.$get("~/api/financial_statements/count?",param,(result)=>{
	  			if(result){
	  				if(result.result >= this.obj.financial_statements_limit_times){
	  					this.financial_statements_limit = true;
	  				}else{
	  					this.financial_statements_limit = false;
	  				}
	  			}
	  		})
	  	}else{
        ...
        this.get_praise();
        this.check_collect();
				this.get_appointment_to_see_the_room_limit();
			this.get_financial_statements_limit();
		      }, 1000);
		},
		watch:{
			obj:{
