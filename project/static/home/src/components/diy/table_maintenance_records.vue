    ...
    <el-form label-position="right" :model="query" class="form p_4" label-width="120">
      <el-row class="rows row1">
                      ...
                      <el-form-item label="维修状态">
                                                      <el-select v-model="query.maintenance_status">
                                                                  <el-option v-for="o in list_maintenance_status" :key="o" :label="o"
                                             :value="o">
                                  </el-option>
                                                            </el-select>
                                                </el-form-item>
      ...
      <el-row class="rows row2">
         ...
         <el-col :xs="24" :sm="10" :lg="8" class="search_btn_1 search_btn_wrap_1 btns">
            ...
            <el-button v-if="$check_action('/maintenance_records/table','del') || $check_action('/maintenance_records/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
                        </el-col>
	    ...
	    <el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
	                <el-table-column fixed type="selection" tooltip-effect="dark" width="55">
                      ...
                      {{ $toTime(scope.row["repair_date"],"yyyy-MM-dd") }}
                                              ...
                                              <el-table-column prop="contents_of_repair_report" @sort-change="$sortChange" label="报修内容"                                v-if="$check_field('get','contents_of_repair_report')" min-width="200">
                                </el-table-column>
                                              <el-table-column prop="maintenance_status" @sort-change="$sortChange" label="维修状态"                                v-if="$check_field('get','maintenance_status')" min-width="200">
                                </el-table-column>
                                              <el-table-column prop="maintenance" @sort-change="$sortChange" label="维修情况"                                v-if="$check_field('get','maintenance')" min-width="200">
                                </el-table-column>
      ...
      <el-table-column sortable prop="update_time" label="更新时间" min-width="200">
        <template slot-scope="scope">
          {{ $toTime(scope.row["update_time"],"yyyy-MM-dd hh:mm:ss") }}
                ...
                <el-table-column fixed="right" label="操作" min-width="120" v-if="$check_action('/maintenance_records/table','set') || $check_action('/maintenance_records/view','set') || $check_action('/maintenance_records/view','get')
					 ">
        

      <template slot-scope="scope">
        <router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
                     v-if="$check_action('/maintenance_records/table','set') || $check_action('/maintenance_records/view','set') || $check_action('/maintenance_records/view','get')"
                                 :to="'./view?' + field + '=' + scope.row[field]"
                                 size="small">
          <span>详情</span>
        </router-link>
                                        		  		  		  	<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="changeSigning(scope.row, scope.$index)" v-if="$check_option('/maintenance_records/table', 'examine')">
		  		<span>审核</span>
		  	</el-button>
		        </template>
    </el-table-column>
					...
					if (valid) {
						...
						if (this.batchAllState) {
							for (var i = 0; i < this.batchAllList.length; i++) {
								let url = "~/api/maintenance_records/set?maintenance_records_id=" + this.batchAllList[i]["maintenance_records_id"];
							...
							var url = "~/api/maintenance_records/set?maintenance_records_id=" + this.verifyItem['maintenance_records_id']
							_this.$post(url, {
								'examine_state': _this.verifyItem.examine_state,
																'examine_reply': _this.verifyItem.examine_reply
															}, (json, status) => {
