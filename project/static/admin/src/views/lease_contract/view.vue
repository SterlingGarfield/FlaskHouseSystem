<template>
	<el-main class="bg edit_wrap comtable_e">
		...
		<el-row class="row_ce"> 
					...
					<div v-else>
						<el-upload v-if="(form['lease_contract_id'] && $check_field('set','contract_documents')) || (!form['lease_contract_id'] && $check_field('add','contract_documents'))":file-list="contract_documents_FileList" :on-preview="openPreview" :on-remove="contract_documentsRemove" class="upload-demo" drag
		...
		toFixed: function(num){
          ...
          this.form["signing_time"] = this.$toTime(parseInt(this.form["signing_time"]),"yyyy-MM-dd")
			...
			get_obj_after(json, func){
																																																																																										if(this.form["signing_time"]=="0000-00-00"){
				  this.form["signing_time"] = null;
				}
				if(parseInt(this.form["signing_time"]) > 9999){
					this.form["signing_time"] = this.$toTime(parseInt(this.form["signing_time"]),"yyyy-MM-dd")
				}
														this.contract_documents_FileList = []
				if (this.form["contract_documents"]) {
					this.contract_documents_FileList = [
					  {
						url: this.$fullUrl(this.form['contract_documents']),
						name: this.form['contract_documents'],
					  },
					];
				}
