<template>
	...
	<el-row class="rows row2">
		...
		<el-col :xs="24" :sm="24" :lg="24" class="search_btn_wrap search_btns">
			...
			<el-table-column fixed="right" label="操作" min-width="200" v-if="$check_action('/lease_contract/table','set') || $check_action('/lease_contract/view','set') || $check_action('/lease_contract/view','get') 
				...
				<template slot-scope="scope">
					...
					v-if="$check_action('/lease_contract/table','set') || $check_action('/lease_contract/view','set') || $check_action('/lease_contract/view','get')"
						...
						<!--跨表按钮-->
										<el-button class="e-button el-button--small is-plain el-button--primary" style="margin: 5px !important;" size="small" @click="to_table(scope.row,'/maintenance_records/view')" v-if="($check_action('/maintenance_records/table','add') || $check_action('/maintenance_records/view','add')) && !scope.row['maintenance_records_limit']">
						<span>维修申请</span>
					</el-button>
													<el-button class="e-button el-button--small is-plain el-button--primary" style="margin: 5px !important;" size="small" @click="to_table(scope.row,'/complaint_records/view')" v-if="($check_action('/complaint_records/table','add') || $check_action('/complaint_records/view','add')) && !scope.row['complaint_records_limit']">
						<span>在线投诉</span>
					</el-button>
												<el-button class="e-button el-button--small is-plain el-button--primary" style="margin: 5px !important;" size="small" @click="openPayModal(scope.row)" v-if="scope.row.pay_state==='未支付' && ($check_pay('/lease_contract/table'))">
						<span>支付</span>
					</el-button>
					</div>
				</template>
