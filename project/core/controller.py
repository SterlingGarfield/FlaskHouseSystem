...
class Controller:
    ...
    def Get_list(self, ctx):
        ...
        if "sqlwhere" in query:
            config_plus["sqlwhere"] = query.pop("sqlwhere")
        count = self.service.Count(query,config_plus)
        lst = []
        if self.service.error:
            return {"error": self.service.error}
        elif count:
    ...
    # 标签推荐
    def Get_tag_list(self, ctx):
        return {"code": 0}

    # 算法推荐
    def Get_algorithm_list(self, ctx):
        return { "code": 0 }
    # 商家查询
